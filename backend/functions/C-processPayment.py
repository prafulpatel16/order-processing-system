...
def lambda_handler(event, context):
    try:
        ...
        # Ensure productId, quantity, and amount are present
        product_id = event.get('productId')
        quantity = event.get('quantity')
        amount = event.get('amount')
        payment_method = event.get('paymentMethod')  # Get the paymentMethod from the event
        ...
        params = {
            ...
            'Item': {
                ...
                'amount': {'N': str(amount)},
                'paymentMethod': {'S': payment_method}  # Save the paymentMethod in DynamoDB
            }
        }
        ...
        return {
            ...
            'quantity': quantity,
            'paymentMethod': payment_method  # Return paymentMethod in response
        }
